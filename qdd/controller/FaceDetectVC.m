...
@interface FaceDetectVC()<CaptureManagerDelegate>
{
    UILabel *alignLabel;
    int number;//
    int takePhotoNumber;
...
-(void)viewDidLoad{
    ...
    self.leftButton.frame = CGRectMake(30*WIDTH_SCALE, 31, 60, 22);
...
-(void)makeUI
{
    self.previewView = [[UIView alloc]initWithFrame:CGRectMake(0, 64, ScreenWidth, ScreenHeight*3/5)];
...
//    [self.view addSubview:imgView];
    ...
    self.textLabel.text = @"请对准屏幕，保持光线充足";
    self.textLabel.textColor = BlueRGBColor;
    [self.view addSubview:self.textLabel];
    
    //背景View
    backView = [[UIView alloc]initWithFrame:CGRectMake(0, 64, ScreenWidth, ScreenHeight-65)];
//    backView.backgroundColor = [UIColor lightGrayColor];
    ...
    //图片放置View
    imageView = [[UIImageView alloc]initWithFrame:CGRectMake(0, 65, ScreenWidth, ScreenWidth*4/3)];
...
-(BOOL)identifyYourFaceLeft:(CGFloat)left right:(CGFloat)right top:(CGFloat)top bottom:(CGFloat)bottom
{
    ...
    if (flag==1) {
        self.textLabel.text = @"人脸识别成功,请张嘴";
        flag++;
    }
...
-(void)identifyYourFaceOpenMouth:(NSString *)key p:(CGPoint )p
{
    ...
    if (rightX && leftX && upperY && lowerY && isJudgeMouth != YES) {
        ...
        if (mouthWidth && mouthWidthF) {
            ...
            //张嘴验证完毕
            if (mouthHeight - mouthHeightF >= 20  && mouthWidthF - mouthWidth >= -1) {
                isJudgeMouth = YES;
...
-(void)identifyYourFaceShakeHead:(NSString *)key p:(CGPoint )p
{
    if ([key isEqualToString:@"mouth_middle"] && isJudgeMouth == YES) {
        ...
        //摇头验证完毕
        if (bigNumber - smallNumber > 30
            ) {
            
            static int flag =1;
            if (flag==1) {
                self.textLabel.text=@"摇头成功.....";
                flag++;
